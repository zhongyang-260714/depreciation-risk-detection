#!/usr/bin/env python3
"""Single-company AI annotation for A-share. 
Usage: python3 annotate_one_cn.py <company_name> <stock_code>
Example: python3 annotate_one_cn.py 寒武纪 688256.SH
"""
...
def run_annotation(company_name: str, stock_code: str, year: int, client: DeepSeekClient) -> dict:
    ...
    passed = 0; failed = 0; verif_results = []
    for sig in signals:
        excerpt = sig.get("text_excerpt", "")
        sig_id = sig.get("signal_id", "UNKNOWN")
        is_noise = False; noise_reason = ""
        if ("公司概况" in excerpt or "公司简介" in excerpt or "公司基本情况" in excerpt) and \
           not any(k in excerpt for k in ["折旧", "摊销", "减值", "固定资产", "使用寿命"]):
            is_noise = True; noise_reason = "公司概况章节误匹配"
        if ("前瞻性" in excerpt or "风险提示" in excerpt or "免责声明" in excerpt) and \
           not any(k in excerpt for k in ["折旧", "摊销", "减值", "固定资产"]):
            is_noise = True; noise_reason = "前瞻性声明/风险提示泛泛提及"
        if ("管理层讨论" in excerpt or "经营情况讨论" in excerpt) and \
           not any(k in excerpt for k in ["折旧年限", "使用寿命", "预计使用年限", "折旧方法", "固定资产"]):
            is_noise = True; noise_reason = "管理层讨论中顺带提及"
        if excerpt and excerpt[:30] in full_text and not is_noise:
            passed += 1; verif_results.append({"signal_id": sig_id, "passed": True, "method": "substring", "confidence": 1.0})
        elif is_noise:
            failed += 1; verif_results.append({"signal_id": sig_id, "passed": False, "method": "noise_excluded", "confidence": 0.0, "reason": noise_reason})
        else:
            failed += 1; verif_results.append({"signal_id": sig_id, "passed": False, "method": "substring", "confidence": 0.0})
    total = len(signals)
    verification = {"passed": passed, "failed": failed, "total": total,
        "pass_rate": passed / total if total > 0 else 0,
        "results": verif_results, "failed_signals": [s for s, v in zip(signals, verif_results) if not v["passed"]]}
    ...
    output = {
        "metadata": {
            ...
            "annotator": "DeepSeek AI + Program Verification (Single Company)",
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 annotate_one_cn.py <company_name> <stock_code>")
        print("Example: python3 annotate_one_cn.py 寒武纪 688256.SH")
        sys.exit(1)
