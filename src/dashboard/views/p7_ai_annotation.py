...
def render(data: dict) -> None:
    ...
    with st.expander("📋 必要说明（P7 功能介绍与使用注意事项）", expanded=False):
        ...
        **二、美股 10-K 模式**
        ...
        - **选择已有公司**：系统自动从 `data/annotated/` 加载已确认的 31 份标注（10 家公司 × 3 财年），可直接选择并运行 AI 验证
        ...
        **三、A股年报模式**
        ...
        - **财年说明**：当前标注数据覆盖 **FY2024**（2024 年年度报告，发布时间 2025 年 3-5 月）。若需跨年轨迹对比（类似美股的 2-3 个财年），理论上可额外下载 FY2022 和 FY2023 年报（均已发布），但工作量较大（10 家 × 3 年 = 30 份 PDF）。
        ...
        **四、常见问题与注意事项**
        ...
        st.caption("📌 注：前六家公司的 FY2024 已有初步人工标注，新增四家（光环新网、海光信息、工业富联、润泽科技）待 AI 标注。下载后可通过 P7 进行 AI 验证与对照。FY2022 和 FY2023 用于 P3「跨年轨迹」展示（可选）。")
    # ---- 输入区 ----
