...
class FeatureEngineer:
    ...
    def build_traditional_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 研发资本化率
        if 'capitalized_rd' in df.columns and 'expensed_rd' in df.columns:
            total_rd = df['capitalized_rd'] + df['expensed_rd']
            df['rd_capitalization_rate'] = df['capitalized_rd'] / total_rd.replace(0, np.nan)
        else:
            df['rd_capitalization_rate'] = np.nan
